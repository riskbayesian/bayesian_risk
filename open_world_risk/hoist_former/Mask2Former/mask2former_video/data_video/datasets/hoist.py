...
__all__ = ["load_hoist_json", "register_hoist_instances"]

HOIST_CATEGORIES = [
    {"color": [106, 0, 228], "isthing": 1, "id": 1, "name": "hand_held_object"},
]

def _get_hoist_instances_meta():
    thing_ids = [k["id"] for k in HOIST_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in HOIST_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 1, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in HOIST_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def load_hoist_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        for frame_idx in range(record["length"]):
            ...
            for anno in anno_dict_list:
                ...
                _hand_masks = anno.get("hand_masks", None)
                _contact_masks = anno.get("contact_masks", None)
                
                if not (_bboxes and _segm and _bboxes[frame_idx] and _segm[frame_idx]):
                    continue

                bbox = _bboxes[frame_idx]
                hand_mask = _hand_masks[frame_idx]
                contact_mask = _contact_masks[frame_idx]
                ...
                if isinstance(hand_mask, dict):
                    if isinstance(hand_mask["counts"], list):
                        # convert to compressed RLE
                        hand_mask = mask_util.frPyObjects(hand_mask, *hand_mask["size"])
                elif hand_mask:
                    # filter out invalid polygons (< 3 points)
                    hand_mask = [poly for poly in hand_mask if len(poly) % 2 == 0 and len(poly) >= 6]
                obj["hand_mask"] = hand_mask

                if isinstance(contact_mask, dict):
                    if isinstance(contact_mask["counts"], list):
                        # convert to compressed RLE
                        contact_mask = mask_util.frPyObjects(contact_mask, *contact_mask["size"])
                elif contact_mask:
                    # filter out invalid polygons (< 3 points)
                    contact_mask = [poly for poly in contact_mask if len(poly) % 2 == 0 and len(poly) >= 6]
                obj["contact_mask"] = contact_mask
...
def register_hoist_instances(name, metadata, json_file, image_root):
    """
    ...
    DatasetCatalog.register(name, lambda: load_hoist_json(json_file, image_root, name))
...
root = './data/HOIST/'
metadata = _get_hoist_instances_meta()
splits = ["train", "test", "valid"]
for splt in splits:
    json_file = os.path.join(root, f'{splt}.json')
    image_root = os.path.join(root, splt, 'JPEGImages')
    register_hoist_instances(f'hoist_{splt}', metadata, json_file, image_root)
