...
def process_numpy_imgs(numpy_images: np.ndarray, save: bool = False, batch_size_processing: int = 3) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
    ...
    weights_path = './open_world_risk/hoist_former/pretrained_models/trained_model.pth'
    cfg_file = './open_world_risk/hoist_former/configs/hoist/hoistformer.yaml'
    ...
    num_batches = (len(frames_list) + batch_size_processing - 1) // batch_size_processing
    with tqdm(total=num_batches, desc="Batch Processing", unit="batch") as pbar:
                ...
                if len(batch_scores) > 0 and np.any(keep_mask):
                    ...
                    for frame_idx_in_batch in range(len(batch_frames)):
                        ...
                        if frame_idx_in_batch < len(batch_frame_masks):
                            ...
                            masks = frame_masks_filtered.cpu().numpy()
                            #print("masks returned shape ", masks.shape)
                            all_masks.append(masks)
                        else:
                            # No masks for this frame
                            all_scores.append(np.array([]))
                            all_labels.append(np.array([]))
                            empty_mask = np.array([]).reshape(0, frame_mask_tensors.shape[1], frame_mask_tensors.shape[2])
                            #print('empty_mask 1 added ', empty_mask.shape)
                    ...
                    for _ in range(len(batch_frames)):
                        all_scores.append(np.array([]))
                        all_labels.append(np.array([]))
                        # Create empty mask with proper shape [0, H, W]
                        if len(batch_masks) > 0 and len(batch_masks[0]) > 0:
                            mask_shape = batch_masks[0][0].shape
                            empty_mask = np.array([]).reshape(0, mask_shape[0], mask_shape[1])
                            #print("empty_mask 2 added ", empty_mask.shape)
                            all_masks.append(empty_mask)
                        else:
                            # Fallback if we can't determine shape
                            empty_mask = np.array([]).reshape(0, numpy_images.shape[1], numpy_images.shape[2])
                            #print("empty_mask 3 added ", empty_mask.shape)
                            all_masks.append(empty_mask)  # Default shape
