...
class RiskField(torch.nn.Module):
    def __init__(self,
                 clip_dim: int,
                 num_layers: int,
                 num_neurons: int,
        ...
        if compute_covariances:
            self.output_dim = self.alphas_dim + 9
        else:
        ...
        modules = [torch.nn.Linear(2*projection_dim, num_neurons),  # Start with Linear
            ...
            torch.nn.ReLU()]
        modules.extend([torch.nn.Linear(num_neurons, num_neurons),
            torch.nn.LayerNorm(num_neurons),
            torch.nn.ReLU()] * (num_layers - 1))
    ...
    def forward(self, clip_coord1: torch.Tensor, clip_coord2: torch.Tensor) -> torch.Tensor:
        ...
        # NOTE: WE CAN TRY DIFFERENT SYMMETRIC FEATURES AS WELL
        out1 = self.mlp(torch.cat([attended[:,0,:], attended[:,1,:]], dim=-1))
        out2 = self.mlp(torch.cat([attended[:,1,:], attended[:,0,:]], dim=-1))
        output = 0.5 * (out1 + out2)  # invariant by construction
        ...
        if self.compute_covariances:
            lambdas_input = output[..., self.alphas_dim:self.alphas_dim+3]
            lambdas_input = torch.softmax(lambdas_input, dim=-1)
            rot_vecs1 = output[..., self.alphas_dim+3:self.alphas_dim+6]
            rot_vecs2 = output[..., self.alphas_dim+6:self.alphas_dim+9]
            rot_vecs1 = rot_vecs1 / (torch.norm(rot_vecs1, dim=-1, keepdim=True) + 1e-8)
            rot_vecs2 = rot_vecs2 / (torch.norm(rot_vecs2, dim=-1, keepdim=True) + 1e-8)
            y_vec = torch.cross(rot_vecs1, rot_vecs2)
            y_vec = y_vec / (torch.norm(y_vec, dim=-1, keepdim=True) + 1e-8)
            z_vec = torch.cross(rot_vecs1, y_vec)
            rots = torch.stack([rot_vecs1, y_vec, z_vec], dim=-1)
            covariances = torch.einsum('bij, bjk, bkl->bil', rots, torch.diag_embed(lambdas_input), rots.transpose(-2, -1))

        return alphas, covariances
